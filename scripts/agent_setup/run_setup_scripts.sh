...
REPO_ROOT="${RTRACE_REPO_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}"
CONFIG_PATH="${1:-${RTRACE_AGENT_SETUP_CONFIG:-$REPO_ROOT/agent_setup/setup.yml}}"

if [[ "${RTRACE_SKIP_AGENT_SETUP:-0}" == "1" ]]; then
    echo "[agent-setup] Skipped (RTRACE_SKIP_AGENT_SETUP=1)"
    exit 0
fi
