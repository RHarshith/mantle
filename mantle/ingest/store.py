...
class TraceStore:
    ...
    def _sync_trace_to_sqlite(self, state: TraceState) -> None:
        ...
        # Persist token byte profile for the profiler view
        profile = self._compute_token_profile(state, turns)
        self.sqlite_store.replace_token_profile(state.trace_id, profile)
    ...
    def _compute_token_profile(self, state: TraceState, turns: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Compute per-turn byte metrics from extracted context and action details.

        For each turn:
          request_bytes(t)  = context tab contents (replay_context_sections or prompt_sections)
          response_bytes(t) = action tab contents (replay_action_sections/response_sections + assistant_text + tool_pairs)
          delta(t) = size of tool call results
        """
        profile: list[dict[str, Any]] = []
        
        actual_turns = [t for t in turns if str(t.get("turn_id", "")) != "setup"]
        for i, td in enumerate(actual_turns):
            req_sections = td.get("replay_context_sections") or []
            if not req_sections:
                req_sections = td.get("prompt_sections") or []
                
            resp_sections = td.get("replay_action_sections") or []
            if not resp_sections:
                resp_sections = td.get("response_sections") or []

            has_assistant_section = any(str(section.get("id") or "") in {"assistant_text", "assistant_messages", "response"} for section in resp_sections)
            response_text = str(td.get("response_text") or "").strip()
            
            tool_pairs = td.get("_tool_pairs") or []
            
            req_b = len(json.dumps(req_sections, ensure_ascii=False).encode("utf-8"))
            
            resp_data: dict[str, Any] = {"sections": resp_sections}
            if response_text and not has_assistant_section:
                resp_data["assistant_text"] = response_text
                
            resp_b = len(json.dumps(resp_data, ensure_ascii=False).encode("utf-8"))
            
            delta = 0
            for tp in tool_pairs:
                res = tp.get("result")
                if res is not None:
                    delta += len(json.dumps(res, ensure_ascii=False).encode("utf-8"))
    ...
    def token_profile(self, trace_id: str) -> dict[str, Any]:
        ...
        self._get_trace(trace_id)  # raises KeyError if not found
        profile = self.sqlite_store.get_token_profile(trace_id)
        return {
            "trace_id": trace_id,
            "profile": profile,
        }
